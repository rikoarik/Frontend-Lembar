...
const ACTIVE_WORKSPACE_COOKIE = 'lembar_active_workspace';
...
function isValidSession(session: string): boolean {
  if (
    ...
    ['demo', 'admin', 'ops'].includes(session)
  ) {
    return true;
  }
  return isFreshJwt(session);
}
...
export function proxy(request: NextRequest): NextResponse {
  ...
  const session =
    request.cookies.get(JWT_COOKIE)?.value ?? request.cookies.get(LEGACY_SESSION_COOKIE)?.value;
  ...
  if (isAuthPage && session && isValidSession(session)) {
