...
export async function DELETE(_request: NextRequest, context: { params: Promise<{ templateId: string }> }) {
  ...
  const upstream = await backendFetch(`/v1/templates/${encodeURIComponent(templateId)}`, { method: 'DELETE', token: auth.token });
  return new NextResponse(upstream.status === 204 ? null : await upstream.text(), { status: upstream.status, headers: upstream.status === 204 ? undefined : { 'content-type': upstream.headers.get('content-type') ?? 'application/json' } });
}
