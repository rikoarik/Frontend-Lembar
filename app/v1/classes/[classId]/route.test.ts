...
const { backendFetch, liveClaims } = vi.hoisted(() => ({ backendFetch: vi.fn(), liveClaims: vi.fn() }));
...
describe('DELETE /v1/classes/[classId]', () => {
  it('forwards class deletion to the backend', async () => {
    ...
    const response = await route.DELETE(new Request('http://localhost/v1/classes/class-1', { method: 'DELETE' }) as never, {
      params: Promise.resolve({ classId: 'class-1' }),
    });

    expect(backendFetch).toHaveBeenCalledWith('/v1/classes/class-1', expect.objectContaining({ method: 'DELETE', token: 'jwt' }));
