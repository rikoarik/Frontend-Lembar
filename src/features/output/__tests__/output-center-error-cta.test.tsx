...
describe('OutputCenterView error CTA', () => {
  it('hides Ke finalisasi when error is 401', async () => {
    mockService.getOutput.mockResolvedValue(
      err({
        code: 'AUTH_REQUIRED',
        safeMessage: 'Silakan masuk dulu.',
        retryable: false,
        httpStatus: 401,
      }),
    );
  ...
  it('shows Ke finalisasi for other errors', async () => {
    mockService.getOutput.mockResolvedValue(
      err({ code: 'UNKNOWN', safeMessage: 'Coba lagi.', retryable: true }),
    );
    render(<OutputCenterView assessmentId="asm-1" />);
