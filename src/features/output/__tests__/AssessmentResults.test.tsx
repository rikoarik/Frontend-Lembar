...
describe('AssessmentResults', () => {
  it('menampilkan hasil, unduhan CSV, dan tombol kembali', async () => {
    render(<AssessmentResults assessmentId="asm-1" />);
    expect(await screen.findByText('Budi')).toBeInTheDocument();
    expect(screen.getByText('7A')).toBeInTheDocument();
    expect(screen.getByText('8 / 10')).toBeInTheDocument();
    expect(screen.getByText(/perlu dinilai/i)).toBeInTheDocument();
    expect(screen.getByRole('link', { name: /csv/i })).toHaveAttribute(
      'href',
      '/v1/assessments/asm-1/results.csv',
    );
