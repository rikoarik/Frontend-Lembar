...
function fetchOutputCenterData(assessmentId: string) {
  return Promise.all([
...
export function OutputCenterView({ assessmentId }: { assessmentId: string }) {
  ...
  const applyLoadResult = useCallback(([outResult, detailResult]: OutputCenterResult) => {
    if (!outResult.ok) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    applyLoadResult(await fetchOutputCenterData(assessmentId));
  }, [applyLoadResult, assessmentId]);

  useEffect(() => {
    let cancelled = false;

    void fetchOutputCenterData(assessmentId).then((result) => {
          ...
          <Panel title="Daftar soal" description={`${detail.questions.length} soal`}>
            <ol className="flex flex-col gap-4">
              {detail.questions.map((q) => (
                <li
                  key={q.id}
                  className="flex gap-3 border-b border-brand-line pb-4 last:border-0 last:pb-0"
                >
                  <span
