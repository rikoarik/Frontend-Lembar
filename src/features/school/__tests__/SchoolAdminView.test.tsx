...
describe('SchoolAdminView honest contract rendering', () => {
  it('replaces unknown school metadata with neutral Indonesian labels', async () => {
    vi.mocked(schoolService.dashboard).mockResolvedValue({
      ok: true,
      value: {
        workspace: { name: 'SD Uji', level: 'unknown' },
        memberCount: 0,
        members: [],
    ...
    render(
      <AdminPanelProvider panelId="summary-labels">
        <SchoolAdminView />
      </AdminPanelProvider>,
    );
    ...
    vi.mocked(schoolService.settings).mockResolvedValue({
      ok: true,
      value: {
        name: 'SD Uji',
        slug: 'sd-uji',
    ...
    render(
      <AdminPanelProvider panelId="settings-labels">
        <SchoolAdminView section="pengaturan" />
      </AdminPanelProvider>,
    );
  ...
  it('does not label live Guru or Penggunaan tables as data preview', async () => {
    ...
    vi.mocked(schoolService.usage).mockResolvedValue({
      ...
      value: {
        quotaUsed: 1,
        quotaLimit: 10,
        breakdown: [{ userId: 'm1', email: 'guru@uji.id', name: 'Guru Uji', used: 1 }],
        trend: [],
      },
    });
    render(
      <AdminPanelProvider panelId="usage-live">
  ...
  it('uses Indonesian wording for pending invitations and notification statuses', async () => {
    ...
    const invitations = render(
      <AdminPanelProvider panelId="invitations-label">
        <SchoolAdminView section="undangan" />
      </AdminPanelProvider>,
    );
    expect(await screen.findByText(/tidak ada undangan menunggu/i)).toBeInTheDocument();
    expect(screen.queryByText(/pending/i)).not.toBeInTheDocument();
    invitations.unmount();
    vi.mocked(schoolService.notifications).mockResolvedValue({
      ok: true,
      value: { data: [], meta: { total: 0, page: 1, limit: 20, pages: 1 } },
    });
    render(
      <AdminPanelProvider panelId="notification-label">
        <SchoolAdminView section="notifikasi" />
      </AdminPanelProvider>,
    );
        ...
        data: [
          {
            ...
            actor: 'Admin',
            action: 'update',
            target: { id: 'member-1' } as unknown as string,
            metadata: null,
          },
        ],
        meta: { total: 1, page: 1, limit: 20, pages: 1 },
