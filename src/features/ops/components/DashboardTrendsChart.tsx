...
export function DashboardTrendsChart() {
  ...
  function MiniBar({
    ...
    if (data.length === 0) {
      return (
        <div className="flex h-20 items-center justify-center rounded-xl bg-[#faf8f5] text-[12px] text-[#8a8379]">
              ...
              >
                <div
                  className={`absolute -top-6 text-[10px] font-bold tabular-nums transition-all duration-150 ${
                    isHovered || isMax
                      ...
                      : 'opacity-0 text-[#8a8379] scale-95 group-hover:opacity-100'
                  }`}
                >
                  {d.count}
                </div>
                ...
                <span
                  className={`mt-2 text-[10px] font-medium tabular-nums transition-colors ${
                    isHovered ? 'text-[#171717] font-semibold' : 'text-[#8a8379]'
                  }`}
                >
      ...
      <div className="rounded-2xl border border-[#ddd4c8]/80 bg-white p-5 shadow-[0_2px_12px_rgba(23,23,23,0.01),0_1px_2px_rgba(23,23,23,0.02)] transition-all duration-200 hover:shadow-[0_8px_24px_rgba(23,23,23,0.04)]">
        <div className="flex items-center justify-between border-b border-[#eee6da]/60 pb-3.5">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-[#8a8379]">Total beban job pemrosesan yang dieksekusi</p>
          </div>
          <div className="text-right">
            <span className="text-[18px] font-extrabold tabular-nums text-[#171717]">
              {trends.jobs.reduce((s, d) => s + d.count, 0)}
            </span>
            <span className="block text-[10px] uppercase font-semibold text-[#8a8379]">total job</span>
          </div>
        </div>
        <MiniBar
          data={trends.jobs}
      ...
      <div className="rounded-2xl border border-[#ddd4c8]/80 bg-white p-5 shadow-[0_2px_12px_rgba(23,23,23,0.01),0_1px_2px_rgba(23,23,23,0.02)] transition-all duration-200 hover:shadow-[0_8px_24px_rgba(23,23,23,0.04)]">
        <div className="flex items-center justify-between border-b border-[#eee6da]/60 pb-3.5">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-[#8a8379]">Laporan masalah / kendala dari pengguna</p>
          </div>
          <div className="text-right">
            <span className="text-[18px] font-extrabold tabular-nums text-[#171717]">
              {trends.quality.reduce((s, d) => s + d.count, 0)}
            </span>
            <span className="block text-[10px] uppercase font-semibold text-[#8a8379]">total laporan</span>
          </div>
        </div>
        <MiniBar
          data={trends.quality}
