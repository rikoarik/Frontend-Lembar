...
describe('ShareManager', () => {
  it('menerbitkan tautan attempt dengan ttlSeconds dan mencabut via DELETE', async () => {
    ...
    expect(screen.getByRole('link', { name: /buka asesmen/i })).toHaveAttribute(
      'href',
      '/attempt/tok-1',
    );
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        data: { id: '1', token: 'tok-1', assessmentId: 'asm-1', revokedAt: '2026-08-09' },
      }),
    });
    await userEvent.click(
      screen
        .getAllByRole('button', { name: 'Cabut' })
