...
describe('admin plan route/method contract', () => {
  ...
  it('GETs the canonical admin plan route', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith('/v1/admin/plans', expect.objectContaining({ method: 'GET' }));
  });
  ...
  it('PATCHes a plan with If-Match revision', async () => {
    ...
    expect(fetchSpy).toHaveBeenCalledWith('/v1/admin/plans/free', expect.objectContaining({
      method: 'PATCH',
