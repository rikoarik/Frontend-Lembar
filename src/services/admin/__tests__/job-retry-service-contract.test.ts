...
describe('admin bulk job retry contract', () => {
  ...
  it('POSTs retry-bulk with the retryable status and active search', async () => {
    ...
    await expect(adminService.retryJobsBulk({ status: 'dead_letter', q: 'export' })).resolves.toMatchObject({
      ok: true,
      value: { retried: 3, skipped: 1 },
    });
    expect(fetchSpy).toHaveBeenCalledWith('/v1/admin/jobs/retry-bulk', expect.objectContaining({
      method: 'POST',
